...
trait Expander {
    ...
    fn expand_variant(&self, input: &DeriveInput, cases: &[Case]) -> Result<TokenStream>;
}
...
fn expand_variant(expander: &dyn Expander, input: &DeriveInput) -> Result<TokenStream> {
                ...
                Ok(Case {
                    ...
                    ty: match fields {
                        ...
                        _ => {
                            return Err(Error::new(
                                name.span(),
                                "`variant` component types can only be derived for `enum`s \
    ...
    expander.expand_variant(input, &cases)
}
...
impl Expander for LiftExpander {
    ...
    fn expand_variant(&self, input: &DeriveInput, cases: &[Case]) -> Result<TokenStream> {
        ...
        for (index, Case { ident, ty, .. }) in cases.iter().enumerate() {
            let index_u8 = u8::try_from(index).map_err(|_| {
                Error::new(
                    input.ident.span(),
                    "enums with more than 255 variants not yet supported",
                )
            })?;
...
impl Expander for LowerExpander {
    ...
    fn expand_variant(&self, input: &DeriveInput, cases: &[Case]) -> Result<TokenStream> {
        ...
        for (index, Case { ident, ty, .. }) in cases.iter().enumerate() {
            ...
            if ty.is_some() {
                lowers.extend(quote!(Self::#ident(value) => {
                    #internal::map_maybe_uninit!(dst.tag).write(wasmtime::ValRaw::i32(#index_i32));
                    value.lower(store, options, #internal::map_maybe_uninit!(dst.payload.#ident))
                }));

                stores.extend(quote!(Self::#ident(value) => {
                ...
                lowers.extend(quote!(Self::#ident => Ok(()),));
                stores.extend(quote!(Self::#ident => Ok(()),));
            }
        }
...
impl Expander for ComponentTypeExpander {
    ...
    fn expand_variant(&self, input: &DeriveInput, cases: &[Case]) -> Result<TokenStream> {
        ...
        for (index, Case { attrs, ident, ty }) in cases.iter().enumerate() {
