...
pub(crate) use futures_and_streams::{
    error_drop, future_drop_receiver, future_drop_sender, future_new, future_receive, future_send,
    stream_drop_receiver, stream_drop_sender, stream_new, stream_receive, stream_send,
};
...
impl<T> Default for GuestTask<T> {
    fn default() -> Self {
        Self {
            lower_params: None,
            lift_result: None,
...
pub(crate) extern "C" fn task_wait<T: 'static>(
    cx: *mut VMOpaqueContext,
...
pub(crate) fn enter<T: Send + 'static>(
    ...
    task.lower_params = Some(lower_params);
    task.lift_result = Some(lift_result);
