...
fn host_close_writer<U, S: AsContextMut<Data = U>>(
    ...
    match mem::replace(&mut transmit.read, new_state) {
        ...
        ReadState::GuestReady {
            ...
            let (local_err_ctx, _) = match reader_state_tbl.get_mut_by_rep(err_ctx) {
                Some(r) => {
                ...
                None => {
                    ...
                    reader_state_tbl.insert(err_ctx, LocalErrorContextRefCount(1))?;
                    reader_state_tbl
                        .get_mut_by_rep(err_ctx)
                        .context("retrieving inserted local error context during guest read")?
                }
            };
    ...
    let result = match mem::replace(&mut transmit.read, new_state) {
        ...
        ReadState::Open => {
            ...
            BLOCKED
                ...
                // If the reader is already closed, there is no work to do --
                // return the fact that the reader is closed
    ...
    let result = match mem::replace(&mut transmit.write, new_state) {
        WriteState::GuestReady {
           ...
           // If the writer elected to close the channel after writing, pass along the error context
            if close {
                cx.concurrent_state().table.get_mut(transmit_id)?.write = WriteState::Closed(err_ctx);
            } else {
                unsafe {
        ...
        WriteState::HostReady { accept, close, err_ctx } => {
