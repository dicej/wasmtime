...
impl AsyncConfig {
    ...
    pub fn call_style(&self) -> CallStyle {
        match self {
            ...
            AsyncConfig::All { concurrent: false }
            ...
            AsyncConfig::All { concurrent: true }
            | AsyncConfig::AllExceptImports {
                concurrent: true, ..
            }
            | AsyncConfig::OnlyImports {
...
fn _new(
    ...
    fn build_world_struct(&mut self, resolve: &Resolve, world: WorldId) {
        ...
        let (async_, async__, bounds, await_) = match self.opts.async_.call_style() {
            CallStyle::Async | CallStyle::Concurrent => {
...
impl Wasmtime {
    ...
    fn world_imports_trait(&mut self, resolve: &Resolve, world: WorldId) {
        ...
        if let CallStyle::Async = self.opts.async_.call_style() {
        ...
        let (async_trait, maybe_send) = if let CallStyle::Async = self.opts.async_.call_style() {
            (
        ...
        if !self.opts.skip_mut_forwarding_impls {
            let maybe_maybe_sized = if let CallStyle::Concurrent = self.opts.async_.call_style() {
                ""
            } else {
                "+ ?Sized"
            };
            ...
            let has_concurrent_function = self.import_functions.iter().any(|f| {
                matches!(
                    self.opts.async_.import_call_style(None, &f.func.name),
                    CallStyle::Concurrent
                )
            });
            ...
            for f in self.import_functions.iter() {
                if let Some(sig) = &f.sig {
                    ...
                    uwrite!(self.src, ")");
                    if let CallStyle::Async = self.opts.async_.import_call_style(None, &f.func.name)
                    {
    ...
    fn world_add_to_linker(&mut self, resolve: &Resolve, world: WorldId) {
        ...
        let (host_bounds, data_bounds) =
            if let CallStyle::Concurrent = self.opts.async_.call_style() {
                (
                    format!(
                        "U: Send{}",
                        self.world_host_traits(resolve, world)
                            .into_iter()
                            .map(|s| format!(" + {s}<Data = T>"))
                            .collect::<Vec<_>>()
                            .concat()
                    ),
                    format!(
    ...
    fn generate_add_resource_to_linker(
        ...
        let camel = name.to_upper_camel_case();
        if let CallStyle::Async = opts.async_.drop_call_style(qualifier, name) {
            uwriteln!(
                src,
                "{inst}.resource_async(
                    \"{name}\",
...
impl<'a> InterfaceGenerator<'a> {
    ...
    fn type_resource(&mut self, id: TypeId, name: &str, resource: &TypeDef, docs: &Docs) {
        ...
        if self.types_imported() {
            ...
            if let CallStyle::Async = self.gen.opts.async_.call_style() {
                uwriteln!(self.src, "#[{wt}::component::__internal::async_trait]")
            }
    ...
    fn generate_add_to_linker(&mut self, id: InterfaceId, name: &str) {
        ...
        let is_maybe_async = matches!(self.gen.opts.async_.call_style(), CallStyle::Async);
        if is_maybe_async {
            uwriteln!(self.src, "#[{wt}::component::__internal::async_trait]")
        }
        ...
        uwriteln!(
            ...
            "
                ...
                pub fn add_to_linker_get_host<T>(
                    linker: &mut {wt}::component::Linker<T>,
                    host_getter: impl for<'a> GetHost<&'a mut T, T>,
                ) -> {wt}::Result<()>
                    where {data_bounds}
                {{
            "
    ...
    fn generate_guest_import_closure(&mut self, owner: TypeOwner, func: &Function) {
        ...
        if let CallStyle::Concurrent = &style {
            uwrite!(self.src, "let r = <T as {host_trait}>::{func_name}(host, ");
        } else {
    ...
    fn define_rust_guest_export(
        ...
        let style = self.gen.opts.async_.call_style();
